...
docker run -v $scripts/../conf:/opt/spark/conf --rm --network host $IMAGE_NAME \
    ...
    --conf spark.armada.executor.limit.memory=2Gi \
    --conf spark.armada.executor.request.memory=2Gi \
    --conf spark.armada.driver.limit.memory=2Gi \
    --conf spark.armada.driver.request.memory=2Gi \
    --conf spark.armada.internalUrl="armada://10.244.2.14:50051" \
    --conf spark.home=/opt/spark \
    --conf spark.armada.container.image=$IMAGE_NAME \
    --conf spark.armada.jobSetId="$JOBSET"11 \
