...
echo gbj $ARMADA_SPARK_HOME/target/armada-cluster-manager_${SCALA_BIN_VERSION}-1.0.0-SNAPSHOT.jar
...
SPARK_DIST_CLASSPATH=$ARMADA_SPARK_HOME/target/armada-cluster-manager_${SCALA_BIN_VERSION}-1.0.0-SNAPSHOT.jar:$ARMADA_SPARK_HOME/target/dependency/*  $SPARK_HOME/bin/spark-class org.apache.spark.deploy.ArmadaSparkSubmit \
  --master $ARMADA_MASTER --deploy-mode cluster \
  --jars $ARMADA_SPARK_HOME/target/armada-cluster-manager_${SCALA_BIN_VERSION}-1.0.0-SNAPSHOT.jar \
  --name spark-pi \
